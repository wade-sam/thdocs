(function (window, undefined) {
    var MapsLib = function (options) {
        ...
        //reset filters
        $("#BusinessName").val(self.convertToPlainString($.address.parameter('BusinessName')));
...
//    }
    ...
    MapsLib.prototype.doSearch = function () {
        var self = this;
        self.clearSearch();
        var address = $("#BusinessName").val();
        self.searchRadius = $("#search_radius").val();
        self.whereClause = self.locationColumn + " not equal to ''";
        
        //-----custom filters-----
        var type_column = "'RateingValue'";
        var searchType = type_column + " IN (-1,";
        if ( $("#5stars").is(':checked')) searchType += "5,";
        if ( $("#4stars").is(':checked')) searchType += "4,";
        if ( $("#3stars").is(':checked')) searchType += "3,";
        if ( $("#2stars").is(':checked')) searchType += "2,";
        if ( $("#1stars").is(':checked')) searchType += "1,";
        if ( $("#0stars").is(':checked')) searchType += "0,";
        self.whereClause += " AND " + searchType.slice(0, searchType.length - 1) + ")";
        var type_column = "'BusinessType'";
        var tempWhereClause = [];
        if ( $("#Distributors").is(':checked')) tempWhereClause.push("Distributors/Transporters");
        if ( $("#Farmers").is(':checked')) tempWhereClause.push("Farmers/growers");
        if ( $("#CaringPremises").is(':checked')) tempWhereClause.push("Caring Premises");
        if ( $("#Mobilecaterer").is(':checked')) tempWhereClause.push("Mobile caterer");
        if ( $("#Manufacturer").is(':checked')) tempWhereClause.push("Manufacturers/packers");
        if ( $("#Othercateringpremises").is(':checked')) tempWhereClause.push("Other catering premises");
        if ( $("#Pub").is(':checked')) tempWhereClause.push("Pub/bar/nightclub");
        if ( $("#Restaurant").is(':checked')) tempWhereClause.push("Restaurant/Cafe/Canteen");
        if ( $("#Retailer").is(':checked')) tempWhereClause.push("Retailers - other");
        if ( $("#Retailers").is(':checked')) tempWhereClause.push("Retailers - supermarkets/hypermarkets");
        if ( $("#School").is(':checked')) tempWhereClause.push("School/college/university");
        if ( $("#Takeaway").is(':checked')) tempWhereClause.push("Takeaway/sandwich shop");
        if ( $("#Hotel").is(':checked')) tempWhereClause.push("Hotel/bed & breakfast/guest house");
    ...
    MapsLib.prototype.addrFromLatLng = function (latLngPoint) {
        ...
        self.geocoder.geocode({
            ...
            if (status === google.maps.GeocoderStatus.OK) {
                if (results[1]) {
                    $('#BusinessName').val(results[1].formatted_address);
                    $('.hint').focus();
                    self.doSearch();
                }
            } else {
